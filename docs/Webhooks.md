...
- **`CancelSubscriptionOnCanceled`** -- On `SubscriptionCanceledImmediately` / `SubscriptionCanceledForNonpayment` / `SubscriptionCanceledWithGracePeriod`, sets the local subscription's `ends_at` **and** stores its `cancellation_reason` (`merchant_request` / `payment_failure` / `customer_request`). The nonpayment case (a hard cancellation after payment recovery is exhausted) ends the subscription just like an immediate cancellation. Read the persisted reason off the model (`$subscription->cancellation_reason`), or `$event->cancellationReason` on the dispatched event, to branch your own dunning/win-back UI off it.
